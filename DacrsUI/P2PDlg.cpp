...
void CP2PDlg::OnBnClickedButtonWithd()
{
	...
	if (  strTxFee < 10000  ) {
		::MessageBox( this->GetSafeHwnd() ,_T("Ð¡·Ñ²»×ã") , _T("ÌáÊ¾") , MB_ICONINFORMATION ) ;
		return ;
	}
...
void CP2PDlg::OnBnClickedButtonRech()
{
	...
	{
		::MessageBox( this->GetSafeHwnd() ,_T("½ð¶î²»ÄÜÎª¿Õ") , _T("ÌáÊ¾") , MB_ICONINFORMATION ) ;
		return ;
	}
	CString strShowData ;
	CString addr;
...
{
	// TODO: ÔÚ´ËÌí¼Ó¿Ø¼þÍ¨Öª´¦Àí³ÌÐò´úÂë  PacketP2PSendContract
...
void CP2PDlg::OnBnClickedButtonMale()
{
	...
	CString strDisplay;
	strDisplay.Format(_T("È·¶¨Òª·¢ËÍ¶ÄÔ¼ÃÃ--->½ð¶î:%s"), strTxMoney);
...
void CP2PDlg::OnBnClickedButtonWoman()
{
	...
	CString strDisplay;
	strDisplay.Format(_T("È·¶¨Òª·¢ËÍ¶ÄÔ¼¸ç--->½ð¶î:%s"), strTxMoney);
	COut outdlg(NULL, strDisplay,100);
	if ( IDOK != outdlg.DoModal()){
		return;
	}
